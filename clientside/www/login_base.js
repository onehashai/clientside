...
async function getSites(email){
    return await $.ajax({
        url: "/api/method/bettersaas.bettersaas.doctype.saas_users.saas_users.get_sites",
        type: "GET",
        dataType: "json",
        data:{
            "email":email
 ...
 async function appendSiteOptions(sites){
    ...
    for (var i = 0; i < sites.length; i++) {    
        ...
        option.value = sites[i].site_name;
        option.text = sites[i].site_name;
        siteSelectList.appendChild(option);
    }

}
function isEmailFormatOKRegex(email){
    var re = /\S+@\S+\.\S+/;
    return re.test(email);
}
async function updateEmail(email){
}
async function checkUserNameAndPasswordForAsite(site_name,email,password){
    return wait = await $.ajax({
        url: "/api/method/bettersaas.bettersaas.doctype.saas_users.saas_users.check_user_name_and_password_for_a_site",
        type: "GET",
