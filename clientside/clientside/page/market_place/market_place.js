					...
					Object.keys(res.message).forEach(function(key,index){
						...
						<div class="card-body" id="div${index}">
							<div class="col-2 col-sm-2 col-md-4 px-0 mb-3">
								<img src='${res.message[key].icon}' class="rounded-circle img-fluid" ></img>
							</div>
						...
						if (res.message[key].installed=='true'){
							...
							(`<button type="button" class="btn btn-danger" id="btn${index} " name='${res.message[key].app_name}' value="uninstall" >Uninstall</button>`)
						}
						else{
							document.getElementById(`div${index}`).innerHTML+=
							(`<button type="button" class="btn btn-primary" id="btn${index} " name=${res.message[key].app_name} value="install" >Install</button>`)
						}
					})
				},
				error:(err)=>{
