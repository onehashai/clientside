...
def create_billing_portal_session(customer_id, return_url):
    ...
    try:
        # products_data = fetch_products_and_prices()
        configuration = stripe.billing_portal.Configuration.create(
            ...
            features={
                "subscription_update": {
                    ...
                    "products": [
                        {
                            "product": "prod_RLhLouY1qq0HCG",
                            "prices": ["price_1QSzwfCwmuPVDwVyXeS3zxY1", "price_1QT00KCwmuPVDwVy6zTKxt68"]
                        },
                        {
                            "product": "prod_RLhiSHjfVnIgHJ",
                            "prices": ["price_1QUOcbCwmuPVDwVyE0xKehR6", "price_1QUOdFCwmuPVDwVyGTtsYvyF"]
                        }
                    ]
                },
                "customer_update": {
...
@frappe.whitelist()
def update_subscription_quantity(curr_users):
    country = frappe.conf.country
    if country == 'IN':
        stripe.api_key = frappe.conf.stripe_secret_key_in
    else:
...
class StripeSubscriptionManager:
    def __init__(self, country=""):
        ...
        if self.region == "IN":
            ...
            price = frappe.conf.get("stripe_prices",{}).get("IN", {}).get("products",{}).get("ERP_STARTER", {}).get("prices", {}).get("monthly", {})
            self.trial_price_id = price["price_id"]
        else:
    ...
    def create_subscription(self, customer_id):
        return stripe.Subscription.create(
            customer=customer_id,
            items=[{"price": self.trial_price_id, "quantity": 10}],
